...
import { Challenge, IDL } from "./type";
import {
  _ludexChallengeApi,
  poll,
  transferWrappedSol,
...
export class ChallengeTXClient {
  ...
  async join(_user: string) {
    ...
    console.log("native mint", pool.mint, NATIVE_MINT);
    if (pool.mint.equals(NATIVE_MINT)) {
      console.log("tokenAccount", userTokenAccount.toString());
      const twsInst = await transferWrappedSol(
        user,
        userTokenAccount,
        challenge.entryFee.toNumber(),
        this.connection
      );
      console.log("twsInst", twsInst);
      twsInst.forEach((tx) => this.tx.add(tx));
      console.log("wrap");
    }
  ...
  async getChallenge() {
    ...
    const pool = await this.program.account.pool.fetch(challenge.pool);
    return { ...challenge, mint: pool.mint };
  }
  ...
  static isValidChallengeKey(key: string) {
    try {
